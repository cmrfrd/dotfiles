...
** Keychain
   ...
   #+RESULTS:
   | /run/user/1000/gnupg/S.gpg-agent.ssh | 1757 | /run/user/1000/gnupg/S.gpg-agent:4857:1 |
...
** File manipulation
   ...
   There is a fantatic package elegantly named 'f' that does this all so i don't have to
...
** =company=
   ...
   #+RESULTS:
   | dashboard-refresh-buffer | org-roam-mode | global-company-mode | x-wm-set-size-hint | tramp-register-archive-file-name-handler | table--make-cell-map |
...
* Terminal
  ...
  | lambda | nil | (define-key vterm-mode-map (kbd M-n) 'multi-libvterm-next) | (define-key vterm-mode-map (kbd M-p) 'multi-libvterm-prev) | (defalias 'vterm-send-Cright #'(lambda nil (interactive) (vterm-send-key <right> nil nil t))) | (defalias 'vterm-send-Cleft #'(lambda nil (interactive) (vterm-send-key <left> nil nil t))) | (define-key vterm-mode-map (kbd C-<right>) 'vterm-send-Cright) | (define-key vterm-mode-map (kbd C-<left>) 'vterm-send-Cleft)                   | (define-key vterm-mode-map (kbd C-y) 'ajc/term-paste)                        | (define-key vterm-mode-map (kbd C-q) nil)                      |                                                              |                                                       |                                           |
  ...
  | lambda | nil | (linum-mode 0) | (goto-address-mode) | (setq comint-process-echoes t) | (define-key term-raw-map (kbd C-y) 'ajc/term-paste) | (define-key term-raw-map (kbd <mouse-2>) 'ajc/term-paste) | (define-key term-raw-map (kbd M-o) 'other-window) | (define-key term-raw-map (kbd M-n) 'multi-term-next) | (define-key term-raw-map (kbd M-p) 'multi-term-prev) | (defalias 'term-send-Cr #'(lambda nil (interactive) (term-send-raw-string ))) | (define-key term-raw-map (kbd C-r) 'term-send-Cr) | (defalias 'term-send-tab #'(lambda nil (interactive) (term-send-raw-string 	))) | (define-key term-raw-map (kbd TAB) 'term-send-tab) | (define-key term-raw-map (kbd <tab>) 'term-send-tab) | (defalias 'term-send-Cspc #'(lambda nil (interactive) (term-send-raw-string (string 0)))) | (define-key term-raw-map (kbd M-SPC) 'term-send-Cspc) | (define-key term-raw-map (kbd C-t) 'term-send-Cspc) | (setq indent-tabs-mode nil) | (setq tab-width 0) | (defalias 'term-send-Cright #'(lambda nil (interactive) (term-send-raw-string [1;5C))) | (defalias 'term-send-Cleft #'(lambda nil (interactive) (term-send-raw-string [1;5D))) | (define-key term-raw-map (kbd C-<right>) 'term-send-Cright) | (define-key term-raw-map (kbd C-<left>) 'term-send-Cleft) | (yas-minor-mode -1) | (setq global-hl-line-mode nil) | (setq bidi-paragraph-direction 'left-to-right) |
...
** Setup org
   ...
   #+RESULTS:
   | org-tempo-setup | (lambda nil (delete '(\.pdf\' . default) org-file-apps) (add-to-list 'org-file-apps '(\.pdf\' . zathura %s))) | org-ref-org-menu | org-bullets-mode | (lambda nil (require 'org-tempo)) | #[0 \301\211\207 [imenu-create-index-function org-imenu-get-tree] 2] | evil-org-mode | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-show-all append local] 5] | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-babel-show-result-all append local] 5] | org-babel-result-hide-spec | org-babel-hide-all-hashes | org-eldoc-load | org-ref-setup-label-finders |
...
** Exporting
   ...
   #+RESULTS:
   : ((dot . graphviz-dot) (arduino . arduino) (redis . redis) (php . php) (C . c) (C++ . c++) (asymptote . asy) (bash . sh) (beamer . latex) (calc . fundamental) (cpp . c++) (ditaa . artist) (dot . fundamental) (elisp . emacs-lisp) (ocaml . tuareg) (screen . shell-script) (shell . sh) (sqlite . sql))
...
** Configure =yasnippet=
   ...
   #+RESULTS:
   | company-yasnippet | company-org-roam | company-bbdb | company-semantic | company-cmake | company-capf | company-clang | company-files | (company-dabbrev-code company-gtags company-etags company-keywords) | company-oddmuse | company-dabbrev |
