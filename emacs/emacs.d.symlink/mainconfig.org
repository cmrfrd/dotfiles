:PROPERTIES:
:ID:       0fcd9897-95dc-4aa3-9e16-f6690460af56
:END:
...
most of my development environment configuratin to a single
...
* Use sensible-defaults.el and load custom scripts
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (sensible-defaults/bind-eval-expression)
...
** Keychain
   ...
   #+RESULTS:
   | /tmp/ssh-XXXXXXhu3QkL/agent.2864 | 2866 | /run/user/1000/gnupg/S.gpg-agent:3462:1 |
...
** Bitwarden Secrets
   ...
   #+begin_src emacs-lisp
     (setq bitwarden-bw-executable (executable-find "bw"))
...
** Load up a theme
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package exwm ; cuz doom needs some shit
       :ensure t
       :straight t)
   ...
   #+RESULTS:
   | mainconfig.org | lagranges_theorem.org | *Minibuf-1* | *Messages* | index.mdx | learning_abstract_algebra.org | breaking_down_pairing_based_cryptography.org | test.txt | *dashboard* | *scratch* | *Minibuf-0* | *Echo Area 0* | *Echo Area 1* | *straight-process* | *code-conversion-work* | *Async-native-compile-log* | *Quail Completions* | *gpg-update-tty* | *emacsql-sqlite* | *trigger_polybar* | *Warnings* | *org-src-fontification:emacs-lisp-mode* | *org-src-fontification:python-mode* | *eldoc* | *helm find files* | *helm buffers* | *helm candidates:Emacs Commands history* | *helm candidates:Emacs Commands* | *helm M-x* |
...
**  Golang
   ...
   #+BEGIN_SRC emacs-lisp :results output silent
		 ...
		 ;; (setq gofmt-command (concat go-projectile-tools-path "goimports"))
   ...
   :PROPERTIES:
   :ID:       3b1ca15d-db49-44a6-96cd-94244e2d36f7
   :END:
...
** Python
   ...
   #+END_SRC
	 ...
	 #+RESULTS:
	 | ~/.local/bin | ~/.cabal/bin | /run/wrappers/bin | /home/cmrfrd/.nix-profile/bin | /etc/profiles/per-user/cmrfrd/bin | /nix/var/nix/profiles/default/bin | /run/current-system/sw/bin | /nix/store/n6fy91pdmnxjds3ymikmlwqvyk47p6m7-emacs-gcc-20211119.0/libexec/emacs/29.0.50/x86_64-pc-linux-gnu |
...
** Setup org
		 ...
		 (add-hook 'org-mode-hook
							 (lambda ()
								 (progn
									 ...
									 (require 'langtool)
									 (setq langtool-bin "~/.nix-profile/bin/languagetool")
									 )
								 )
							 )
   #+end_src
	 ...
	 #+RESULTS:
	 | (lambda nil (progn (require 'org-tempo) (auto-fill-mode t) (flyspell-mode t) (use-package langtool :ensure t) (require 'langtool) (setq langtool-bin ~/.nix-profile/bin/languagetool))) | org-bullets-mode | org-eldoc-load | #<subr F616e6f6e796d6f75732d6c616d626461_anonymous_lambda_9> | #[0 \301\211\207 [imenu-create-index-function org-imenu-get-tree] 2] | evil-org-mode | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-show-all append local] 5] | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-babel-show-result-all append local] 5] | org-babel-result-hide-spec | org-babel-hide-all-hashes |
   ...
   #+begin_src emacs-lisp
		 ...
		 (use-package ob-go
			 :ensure t)
		 (setq ob-ipython-auto-configure-kernels t)
...
** Org roam
   ...
   #+begin_src emacs-lisp
     (use-package org-roam
       ...
       (if (emacsql-sqlite-ensure-binary)
       ...
       (defun org+-title-head (title slug)
         (if (> (length title) 0)
             (s-downcase
              (s-replace-all
               '((" " . "_") ("." . "_") ("/" . "_"))
           ...
           (s-downcase
            (s-replace-all
             '((" " . "_") ("." . "_") ("/" . "_"))
             (org-roam--title-to-slug slug)))))
       (defun org+-get-name (title url created)
         (s-concat
       ...
       (defun org+-do-rename-to-title-date ()
         ...
         (let ((new-buffer-filename (format "%s.org"
                                            (org+-get-name
                                             ...
                                             (or "" (org-global-prop-value "URL"))
                                             (org-global-prop-value "CREATED")))))
           (if (not (get-buffer new-buffer-filename))
               (progn
                 (message (format "Renaming file to %s" new-buffer-filename))
                 (rename-file-and-buffer new-buffer-filename)))
       ...
       (setq org-roam-capture-ref-templates
             '(("r" "ref" plain
                "%?"
                :target (file+head
                         "captures/%(org+-title-head \"${title}\" \"${ref}\").org"
                         "#+TITLE: %(org+-title-head \"${title}\" \"${ref}\")\n#+CREATED: %U\n#+LAST_MODIFIED: %U\n#+URL: ${ref}\n#+FILETAGS: website\n")
                :if-new (file+head
                         "captures/%(org+-title-head \"${title}\" \"${ref}\").org"
                         "#+TITLE: %(org+-title-head \"${title}\" \"${ref}\")\n#+CREATED: %U\n#+LAST_MODIFIED: %U\n#+URL: ${ref}\n#+FILETAGS: website\n")
                :unnarrowed t)
       ...
       (setq org-roam-dailies-capture-templates
             '(("J" "Journal" plain
                ...
                :if-new (file+head "%<%Y-%m-%d>.org"
                                   "#+TITLE: Journal entry %U\n#+STARTUP: folded\n#+FILETAGS: journal\n%i")
                :jump-to-captured t :immediate-finish t)))
       (setq org-roam-dailies-directory org-journal-directory)
       )
   #+end_src
   ...
   deft is a thing that's a cool interface for searching through files
...
**** Capturing tasks
     :PROPERTIES:
     :ID:       30587987-2863-47d1-b226-1f3352451043
     :END:
     ...
     #+BEGIN_SRC emacs-lisp :results value verbatim
       ...
       (defun format-filename (filename)
         (f-join org-directory "map"
                 (format "%s.org"
                         (s-replace-all '((" " . "_")) filename))))

       (setq org-capture-templates
             '(("f" "fleeting" plain
                "#+TITLE: ${slug}\n#+CREATED: %U\n#+LAST_MODIFIED: %U\n#+FILETAGS: fleeting\n"
                :target (file "%(format-filename \"${slug}\")")
                :jump-to-captured t)
               ("c" "concept" plain
                "#+TITLE: ${slug}\n#+CREATED: %U\n#+LAST_MODIFIED: %U\n#+FILETAGS: concept\n"
                :target (file "%(format-filename \"${slug}\")")
               ...
               ("j" "Journal entry" plain (function org-journal-find-location)
                ...
                :jump-to-captured t :immediate-finish t)))
       (setq org-roam-capture-templates org-capture-templates)
...
** Exporting
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (defun org-babel-execute-named-src-blocks ()
       (interactive)
       (let ((org-confirm-babel-evaluate (lambda (lang body) (not (string= lang "emacs-lisp")))))
         (let (value)
           (dolist (elt (reverse (org-babel-src-block-names)) value)
             (let ((src-block elt))
               (org-babel-goto-named-src-block src-block)
               (progn
                 (next-line-and-recenter)
                 (previous-line-and-recenter))
               (let ((has-evaluated (org-babel-execute-src-block)))
                 (if has-evaluated
                     (progn
                       (message has-evaluated)
                       (org-babel-goto-named-result src-block)
                       (progn
                         (next-line-and-recenter)
                         (previous-line-and-recenter))
                       (if (not (yes-or-no-p "Continue: "))
                           (error "Exited evaluation ...")))))
               )))))
...
* EXWM
    ...
    (use-package exwm
      ...
      (defun exwm-workspace-previous ()
        ...
        (let ((index exwm-workspace-current-index))
          (if (= index 0)
              (exwm-workspace-switch (- (exwm-workspace--count) 1))
            (exwm-workspace-switch (% (- index 1) (exwm-workspace--count))))))
      ...
      (defun call-tab-prefix-map () (interactive) (call-keymap 'tab-prefix-map "enter key npcd: "))
      (define-key exwm-mode-map [?\C-q] 'call-tab-prefix-map)
  ...
  #+begin_src emacs-lisp
    ...
    (defun generate-randr-config (primary secondary)
      (-flatten `(,(-map (lambda (n) (list n primary)) (number-sequence 1 9))
                  (0 secondary))))

    (setq monit-name "DP-3")
    (defun monitor-home-setup ()
      (interactive)
      (setq exwm-randr-workspace-monitor-plist (generate-randr-config "eDP-1" monit-name))
      (exwm-randr-refresh)
      (shell-command (format "xrandr --output %s --mode 3840x1600 --left-of eDP-1 --auto --primary" monit-name))
      ;(shell-command (format "xrandr --output %s --mode 2560x1080 --left-of eDP-1 --auto --primary" monit-name))
      (trigger_polybar)
    )
      ; (setq exwm-randr-workspace-monitor-plist '(0 monit-name 1 "eDP-1"))
      ...
      ; "xrandr" nil "xrandr --output DP-2 --left-of eDP-1 --auto")
      ;;(add-hook 'exwm-randr-screen-change-hook 'setmon)
                                            ;(shell-command (format "xrandr --output %s --mode 1920x1080i --left-of eDP-1" monit-name))
                                            ;(exwm-randr-enable)

    (defun monitor-home-teardown ()
      (interactive)
      (setq exwm-randr-workspace-monitor-plist '(0 "eDP-1"))
      (shell-command (format "xrandr --output eDP-1 --auto --output %s --off" monit-name))
      (trigger_polybar)
      )
  #+end_src
